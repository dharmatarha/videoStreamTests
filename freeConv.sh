...
if [ "$2" == "Mordor" ]; then
  gstCommand="gst-launch-1.0 -v v4l2src device=/dev/video0 ! image/jpeg,width=1920,height=1080,framerate=30/1 ! jpegdec ! queue ! videoconvert ! rtpvrawpay ! udpsink host=192.168.1.60 port=19009 sync=false"
   ...
   gstCommand="gst-launch-1.0 -v v4l2src device=/dev/video1 ! image/jpeg,width=1920,height=1080,framerate=30/1 ! jpegdec ! queue ! videoconvert ! rtpvrawpay ! udpsink host=192.168.1.1 port=19009 sync=false" 
fi
